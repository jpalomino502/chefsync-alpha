...
const Dashboard = () => {
    ...
    return (
      <div className="relative bg-black h-screen overflow-hidden">
        ...
        />
        <Header toggleSidebar={toggleSidebar} pageTitle={
          activeView === "Summary" ? "Resumen" :
          activeView === "Menu" ? "Menú" :
          activeView === "Command" ? "Comanda" :
          activeView === "Bill" ? "Cuenta" :
          activeView === "ConfigApp" ? "Configuración" : ""
        } />
        <div className={`flex flex-col h-screen pt-16 lg:pt-0 transition-all duration-300 ease-in-out ${isSidebarOpen ? "lg:ml-[320px]" : "lg:ml-[100px]"}`}>
          <main className="flex-1 overflow-auto bg-white lg:rounded-2xl lg:my-2 lg:mr-2">
            <div className="h-full w-full container mx-auto p-4">
              {activeView === "Summary" && <Summary commerceId={commerceId} />}
              {activeView === "Menu" && <Menu commerceId={commerceId} />}
              {activeView === "Command" && <Command commerceId={commerceId} />}
              {activeView === "Bill" && <Bill commerceId={commerceId} />}
              {activeView === "ConfigApp" && <ConfigApp commerceId={commerceId} />}
            </div>
          </main>
        </div>
        {isSidebarOpen && (
          <div className="fixed inset-0 bg-black/50 z-40 lg:hidden transition-opacity duration-300 ease-in-out" onClick={closeSidebar}></div>
        )}
      </div>
    )
  }
